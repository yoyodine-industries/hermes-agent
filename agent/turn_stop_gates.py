...
def _kanban_stop_nudge(agent, messages) -> Optional[str]:
    """Workers must end with a terminal board tool — kanban_complete / kanban_block, or a
    lane handoff (kanban_request_review / kanban_request_changes); a narrated stop is
    recorded as protocol_violation, so nudge once or twice first."""
    try:
