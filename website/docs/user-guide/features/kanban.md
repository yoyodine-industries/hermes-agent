...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree` — a git worktree under `.worktrees/<id>/` for coding tasks. Use `worktree:<path>` to pin the exact target path. Worker-side `git worktree add` creates it, using `--branch` when provided, and stamps it with `KANBAN-WORKTREE.md` so a parked tree is never mistaken for the project checkout (the marker is listed in `.git/info/exclude`, so it never dirties the tree). **Reaped once the card ends, as long as nothing in it is unrecoverable**: commits are protected by *keeping the branch* (a project-linked card's `<slug>/<id>` branch is never deleted), so a clean tree goes even when nothing was pushed; only uncommitted changes, or a detached HEAD holding commits no ref reaches, preserve it. Trees left behind by a card that ended without the completion/archive hook are swept by the dispatcher tick (hourly) and by `hermes kanban gc`.
