...
The dispatcher refuses to re-spawn a ready task when its previous worker process is still alive (`sibling_live`), when it hit a quota/auth/429 error on the previous run (`blocker_auth`), when it completed a run successfully within the guard window (`recent_success`), or when a recent task comment links to a GitHub PR that is still OPEN (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up. See the `respawn_guarded` row in the [event reference](#event-reference).

`active_pr` holds only while that PR is genuinely in flight: the guard asks the forge (`gh pr view <url> --json state`) and releases the card for a MERGED or CLOSED PR — re-spawning under a merged PR risks no duplicate — and for a PR the forge no longer knows (a purged reference is an answer, not a lookup failure). A state it cannot read at all holds the card on the old text-only rule and says so in one WARNING per card. The card is also released when a handoff is recorded *after* the comment that quotes the PR — a reviewer's changes-requested verdict, a review reopen, or the card moving to a different profile — because that PR is then exactly what the card must work on; a PR comment newer than the handoff guards again.

`sibling_live` exists because a released claim is not a dead process: `hermes kanban block` (and reclaim, and a crash requeue) clears the claim while the worker process keeps running — a worker that blocks its own card is mid-turn by definition. Every other signal the dispatcher reads then says "nobody owns this", so it would spawn a second worker over the first: two agents racing the same files and burning tokens on one card. The guard instead reads the `spawned` event — the only spawn record that survives the claim being cleared — and matches its pid *and* process create-time against the live process table, so a reused pid is not mistaken for the worker. The deferral ends when that process really exits, or after two hours, when the spawn record is treated as stale so a leaked pid can never park a card forever. A run that **completed** is exempt: its handoff is already durable and re-running the card is the designed path (`recent_success` owns that window).
...
| Kind | Payload | When |
|---|---|---|
| `spawned` | `{pid, create_time?}` | Dispatcher successfully started a worker process. `create_time` (process start time) is included when readable and lets the respawn guard tell this worker apart from a later process that reused the pid. |
...
| `respawn_guarded` | `{reason}` | Dispatcher refused to re-spawn this ready task this tick. Reasons: `sibling_live` (the previous worker process is still alive — spawning now would race two workers on one card; the claim was released while the process kept running, so the guard waits for it to exit), `blocker_auth` (last failure was a quota/auth/429 error — wait for the rate window to reset), `recent_success` (a completed run happened in the last hour — wait for review before re-running), `active_pr` (an OPEN GitHub PR URL appears in a recent comment — a prior worker already opened a PR; a merged or closed PR, or an OPEN PR the card was re-queued against afterwards, is not a guard reason). The task stays in `ready`; the next tick gets another chance to spawn. If the underlying condition persists, the normal `consecutive_failures` circuit breaker will auto-block via `gave_up` after `failure_limit` failures. |
