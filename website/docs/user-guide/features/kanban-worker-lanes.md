...
- **Per-task max runtime** — `task.max_runtime_seconds` hard-caps wall-clock time per run, regardless of PID liveness. Catches genuinely-deadlocked workers that the live-PID extension would otherwise keep running. A task that sets no cap of its own is still bounded: the dispatcher applies the resolved `kanban.default_max_runtime_seconds` (`7200` by default), so an unbounded card can't wedge a slot forever. Only an explicit per-task cap reaches the worker as a terminal-tool timeout.
