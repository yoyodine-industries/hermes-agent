...
def test_review_dispatch_gate_prevents_phantom_reviewer(
    ...
    with kbc.connect() as conn:
        ...
        kb.claim_task(conn, tid)
        # The reviewer is not the implementer: the self-review guard refuses
        # to spawn a review of the author's own change.
        kb.request_review(
...
def test_active_pr_guard_skipped_for_review_lane_but_defers_ready_lane(
    ...
    with kbc.connect() as conn:
        ...
        assert kb.request_review(
            conn, review_id, summary="PR ready", reviewer="default",
            expected_run_id=claimed.current_run_id,
        )
