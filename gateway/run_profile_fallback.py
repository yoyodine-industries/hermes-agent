...
# Frames that merely *forward* a resolution: the driver is always above them.
_RESOLUTION_FRAMES = frozenset({
    "_resolve_profile_home_for_source", "resolve_profile_home_for_source",
    "_profile_scope_for_source", "profile_scope_for_source",
    "_profile_runtime_scope", "profile_runtime_scope", "__enter__", "__exit__",
})


def driver_caller_label() -> str:
    """``file:line:function`` of the first frame outside the profile-resolution chain.

    ``caller_label`` cannot identify a driver: it is evaluated from inside the argument list of
    the log call, so its caller is always the resolver that is asking, never the loop or turn
    that asked the resolver. Walk past the forwarding frames to name the real caller — the
    difference between "the same line every time" and "here is what loops".
    """
    try:
        ...
        for _ in range(40):
            if frame is None:
                break
            code = frame.f_code
            last = f"{Path(code.co_filename).name}:{frame.f_lineno}:{code.co_name}"
            if code.co_name not in _RESOLUTION_FRAMES:
                return last
            frame = frame.f_back
        return last
    except Exception:
