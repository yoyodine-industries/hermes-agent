...
from tools.protected_instruction_command_guard import TOOL_TERMINAL, protected_instruction_block
...
def _pre_exec_block(
    ...
    then the dangerous-workdir check, then the self-repo guard (local only), then the
    protected-instruction gate (a redirect/tee/interpreter write to a file that steers
    future agents — every backend, and not skippable by ``force``).
    """
    ...
    blocked = protected_instruction_block(
        text=command, tool=TOOL_TERMINAL, task_id=task_id or "default",
    )
    if blocked:
        raise _Rejected(blocked)
