...
from tools.terminal_tool_guards import (
    ...
    gateway_lifecycle_block, self_repo_block, unbounded_search_block,
)
...
def _pre_exec_block(
    ...
    then the dangerous-workdir check, then the unbounded recursive-search
    guard (cheap argv inspection), then the self-repo guard (local only).
    """
    ...
    blocked = unbounded_search_block(
        command=command, env=env, env_type=env_type, cwd=cwd, workdir=workdir, session_key=session_key,
    )
    if blocked:
        raise _Rejected(blocked)
