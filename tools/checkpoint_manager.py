...
DEFAULT_EXCLUDES = [
    ...
    "__KMP_REGISTERED_LIB_*",  # native runtime artifacts (libiomp5 — other-user-owned, breaks `git add -A`)
]
...
class CheckpointManager:
    ...
    def ensure_checkpoint(self, working_dir: str, reason: str = "auto") -> bool:
        ...
        if not self._git_available:
            return False
        abs_dir = str(_normalize_path(working_dir))
        if abs_dir in {"/", str(Path.home()), "/tmp", "/private/tmp"}:  # never snapshot root/home/tmp
