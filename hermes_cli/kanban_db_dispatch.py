...
def board_spawnable_work(
    conn: sqlite3.Connection, *, include_review: bool = True, now: Optional[float] = None,
) -> tuple[float, int]:
    """``(oldest_age_seconds, spawnable_count)`` for one board's spawnable cards.

    Same spawnable filter as :func:`oldest_spawnable_pending` — ``ready`` (or
    ``review`` when review dispatch is on), assigned, unclaimed, and the
    assignee maps to a real Hermes profile (control-plane lanes excluded). The
    gateway dispatcher's cross-board fair-share coordinator uses the COUNT so a
    board whose queue is shorter than its floor share cannot absorb slots it
    will never use, and the OLDEST AGE so scarce slots go to the longest-waiting
    work rather than to board order. ``(0.0, 0)`` when nothing is spawnable.
    """
    statuses = ["ready"]
    if include_review and review_dispatch_enabled():
        statuses.append("review")
    try:
        rows = conn.execute(
            "SELECT assignee, created_at FROM tasks "
            f"WHERE status IN ({','.join('?' for _ in statuses)}) "
            "AND assignee IS NOT NULL AND claim_lock IS NULL "
            "ORDER BY created_at ASC",
            tuple(statuses),
    ...
    for row in rows:
        ...
        count += 1
        created = row["created_at"]
        age = 0.0 if created is None else max(0.0, now - float(created))
        if oldest is None or age > oldest:
            oldest = age
    return (0.0 if oldest is None else oldest), count


def fair_share_spawn_budget(
    host_budget: int, board_work: list[tuple[str, float, int]],
) -> dict[str, int]:
    """Apportion a shared host spawn budget across boards with spawnable work.

    ``board_work`` is ``(slug, oldest_age_seconds, spawnable_count)`` for every
    board that has spawnable work; ``host_budget`` is the number of host worker
    slots free this tick. Returns ``{slug: spawn_cap}`` — each board's ceiling
    for this tick — the cross-board analog of the review-lane reservation in
    ``_dispatch_once_locked``: no single board may consume the entire host
    budget while a sibling board has waiting work. A board with no slot this
    tick is returned with a cap of ``0`` (not omitted) so the dispatcher caps it
    to zero rather than letting board order grab the scarce slot behind the
    live host-cap backstop.

    Policy: a floor share of ``host_budget // n`` per board, then the
    ``host_budget % n`` remainder to the boards with the OLDEST pending work
    first, then any slots freed by count-capping redistributed oldest-first.
    Age-ordered remainder is what keeps the scarce-slot tie-break from degrading
    to board order: when free slots are fewer than boards, the longest-waiting
    work wins — the within-board effective-priority principle applied across
    boards. ``host_budget <= 0`` or empty ``board_work`` returns ``{}``.
    """
    if host_budget <= 0 or not board_work:
        return {}
    n = len(board_work)
    base, remainder = divmod(host_budget, n)
    # Oldest first: the largest age_seconds is the longest-waiting card.
    ordered = sorted(board_work, key=lambda item: item[1], reverse=True)
    caps: dict[str, int] = {}
    leftover = 0
    for i, (slug, _age, count) in enumerate(ordered):
        want = base + (1 if i < remainder else 0)
        # Never allocate more than the board can actually spawn this tick.
        capped = min(want, max(int(count), 0))
        caps[slug] = capped
        leftover += want - capped
    # Redistribute slots freed by count-capping, oldest-first.
    if leftover > 0:
        for slug, _age, count in ordered:
            if leftover <= 0:
                break
            room = max(int(count), 0) - caps[slug]
...
def dispatch_once(
    ...
    *,
    ...
    tick_spawn_cap: Optional[int] = None,
) -> DispatchResult:
...
def _tick_spawn_budget(
    ...
    *,
    ...
    ``tick_spawn_cap`` is the gateway dispatcher's fair-share ceiling for this
    board this tick (see :func:`fair_share_spawn_budget`): it prevents a deep
    queue on an earlier board from consuming the whole host budget and starving
    its siblings. Purely a ceiling — it never grants capacity the other caps
    ...
    if tick_spawn_cap is not None:
        # Coordinator fair-share ceiling. A cap of 0 is meaningful (this board
        # has spawnable work but no slot this tick): it must spawn nothing even
        # though the live host-cap backstop might still show headroom, else the
        # scarce slot would go to board order rather than to the oldest work.
        if spawn_budget is None or spawn_budget > tick_spawn_cap:
            spawn_budget = tick_spawn_cap
